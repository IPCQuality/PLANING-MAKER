...
// === OPTIMIZATION MODULE v2.0 ===
BrainAI.optimizeMachines = function(machines){
  return [...machines].sort((a,b)=>{
    const wsA=(a.wsKey||a.ws||'').length;
    const wsB=(b.wsKey||b.ws||'').length;
    return wsA-wsB;
  });
};
BrainAI.getCoverageReport = function(result){
  const total=result?.machines?.length||0;
  const covered=result?.assignments?.length||0;
  return {total,covered,percent:total?Math.round(covered/total*100):0};
};
BrainAI.saveHistoryJSON = function(){
  return JSON.stringify(JSON.parse(localStorage.getItem('planning_history')||'{}'),null,2);
};
